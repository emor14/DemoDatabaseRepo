...
--processLoadByResourceType '2020-01-01', '2020-04-30'
--go

CREATE procedure [dbo].[processLoadByResourceType] (@startDate date, @endDate date) as
begin
	set nocount on
				
	declare
		--@startDate date = '2020-01-01', --cast(getdate() as date),
		--@endDate date = '2020-04-30', --cast(dateadd(month, 6, getdate()) as date),
		@projectId int,
		@taskUniqueId int,
		@subTaskUniqueId int,
		@executionOrder int,
	...
	declare curTasks cursor for
		...
		where
			...
			p.division_name = 'WORSLEY'/* and
			p.PROJECTID = 379 and
			pt.TASKUNIQUEID = 1583 and
			pt2.TASKUNIQUEID <> 11410*/
		order by
			p.PROJECTID,
			pt.TASKID,
	...
	--select * from #movedTasks order by startDateTime
				 
	select
		@previousResourceTypeId = null,
		...
		begin
			...
			if @@fetch_status = 0
				begin
				 	if
				 	(
						(@previousResourceTypeId is null) or
						(@previousResourceTypeId <> @resourceTypeId) or
				 		(@previousProjectId is null) or
				 		(@previousProjectId <> @projectId) or
				 		(@previousTaskUniqueId is null) or
						...
						(@previousStartDate <> @startDateTime )
				 		...
				 		begin
				 			if
				 			(
								(@previousResourceTypeId is not null) and
				 			...
				 			delete from #wipControl
				 
				 			select
								@previousResourceTypeId = @resourceTypeId,
				 				@previousProjectId = @projectId,
				 				@previousTaskUniqueId = @taskUniqueId,
				 				--@previousStartDate = cast(@startDateTime as date),
								@previousStartDate = @startDateTime,
				 		...
				 		if @wip = @SUBTASK_WIP_LIMIT
				 			begin
				 				...
				 				update
				 					wip1
								set
									duration = round((wip1.duration - wip0.minDuration), 0)
				 				from
				 					#wipControl wip1
				 						inner join #wipControl wip0
				 							on	wip0.resourceTypeId = wip1.resourceTypeId and
				 	...
				 	where
						...
						and resourceTypeId = @resourceTypeId --- new change 23/04/2020
	...
	if exists(select 1 from sysobjects where type = 'u' and name = 'LoadByDayByResourceType_New')
		drop table LoadByDayByResourceType_New
	--else
