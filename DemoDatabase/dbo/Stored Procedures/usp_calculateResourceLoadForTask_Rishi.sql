...
-- =============================================
CREATE PROCEDURE [dbo].[usp_calculateResourceLoadForTask_Rishi] 
	@projectId int,
	@subTaskId int,
	@parentTaskId int,
	...
	begin
		...
		begin
			...
			begin
				...
				--insert into TaskLoadByDay (ProjectId,ParentTaskId,SubtaskId,ResourceTypeId,[date],[Hours],units) 
				--select 	@projectId, @parentTaskId,@subTaskId, @resourceTypeId, cast(@startDatetime as date),@durationForTheDay/3600,@units 
				...
				insert into TaskLoadByDay_Temp
				(	ProjectId
					,ParentTaskId
					,SubtaskId
					,ResourceTypeId
				...
				select 
					...
					,@resourceTypeId
					,@startDatetime
					,@durationForTheDay/3600
					,@units
					,cast(@startDatetime as date)
					,cast(@startDatetime as time(0))
					,cast(@endTimeForDay as time(0))
					,@wiplimit
					,@subtaskType
					,@concSubTaskId
